...
with torch.no_grad():
    for i, batch in enumerate(train_loader):
        ...
        # Tokenizer: max sequence length is passed as argument; always truncate when
        # the tokenized length exceeds max length; otherwise, always pad.
        x = tokenizer(batch["x"], padding='max_length', truncation=True, 
                      max_length=args.max_length, return_tensors="pt")
        y = batch["y"].float()

        # For DNABERT-2, the tokenizer outputs a dictionary containing tokenized IDs and
        # attention masks (0 is padding).
        ...
        if args.pooling == "cls":
            ...
            # Take mean of all non-padding tokens
            embed = torch.sum(attention_mask*embed, axis=1)/torch.sum(attention_mask, axis=1)
        elif args.pooling == "max":
            attention_mask = torch.unsqueeze(x["attention_mask"], dim=-1)
            # Take max of all non-padding tokens for each embedding dimension
...
if args.pooling == "cls":
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/train_embed_dnabert.csv', index=False)
elif args.pooling == "mean":
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/train_embed_dnabert_meanpool.csv', index=False)
else:
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/train_embed_dnabert_maxpool.csv', index=False)
...
with torch.no_grad():
    for i, batch in enumerate(test_loader):
        ...
        if args.pooling == "cls":
            ...
            attention_mask = torch.unsqueeze(x["attention_mask"], dim=-1)
            embed = torch.max(embed * attention_mask + (1 - attention_mask) * -1e9, dim=1)[0]
...
output.columns = [f"embedding_{i}" for i in range(768)] + ["target"]
if args.pooling == "cls":
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/test_embed_dnabert.csv', index=False)
elif args.pooling == "mean":
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/test_embed_dnabert_meanpool.csv', index=False)
else:
    output.to_csv(f'{project_dir}/embeddings/{args.data_name}/test_embed_dnabert_maxpool.csv', index=False)