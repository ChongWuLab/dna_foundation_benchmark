...
def _embed_batch_dnabert2(seq_list, tokenizer, model, device, max_length):
    """Internal function to embed a single batch with the DNABERT-2 model."""
    tok = tokenizer(
        seq_list,
        padding='max_length',
        truncation=True,
...
def get_dnabert2_embeddings(model, tokenizer, device, sequences, batch_size, max_length):
    """Processes a list of sequences in batches to get DNABERT-2 embeddings."""
    all_embeddings_np = []
    ...
    for i in range(0, num_sequences, batch_size):
        ...
        batch_embeddings = _embed_batch_dnabert2(batch_seqs, tokenizer, model, device, max_length)
...
for folder in folders:
    for file_type in file_types:
        ...
        embeddings1 = get_dnabert2_embeddings(model, tokenizer, device, sequences1, BATCH_SIZE, MAX_LENGTH)

        print(f"Generating embeddings for {num_seqs} sequences in Seq2...")
        embeddings2 = get_dnabert2_embeddings(model, tokenizer, device, sequences2, BATCH_SIZE, MAX_LENGTH)
