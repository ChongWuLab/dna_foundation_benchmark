...
def _embed_batch_ntv2(seq_list, tokenizer, model, max_length):
    """Internal function to embed a single batch with the NT-v2 model."""
    tok = tokenizer(
        seq_list,
        padding='max_length',
        truncation=True,
...
def get_ntv2_embeddings(model, tokenizer, sequences, batch_size, max_length):
    """Processes a list of sequences in batches to get NT-v2 embeddings."""
    all_embeddings_np = []
    ...
    for i in range(0, num_sequences, batch_size):
        ...
        batch_embeddings = _embed_batch_ntv2(batch_seqs, tokenizer, model, max_length)
...
print("Loading NT-v2 tokenizer and model...")
...
for folder in folders:
    for file_type in file_types:
        ...
        embeddings1 = get_ntv2_embeddings(model, tokenizer, sequences1, BATCH_SIZE, MAX_LENGTH)

        print(f"Generating embeddings for {num_seqs} sequences in Seq2...")
        embeddings2 = get_ntv2_embeddings(model, tokenizer, sequences2, BATCH_SIZE, MAX_LENGTH)
