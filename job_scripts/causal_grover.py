...
def _embed_batch_grover(seq_list, tokenizer, model, device, max_length):
    """Internal function to embed a single batch with the GROVER model."""
    tok = tokenizer(
        seq_list,
        padding='max_length',
        truncation=True,
...
def get_grover_embeddings(model, tokenizer, device, sequences, batch_size, max_length):
    """Processes a list of sequences in batches to get GROVER embeddings."""
    all_embeddings_np = []
    ...
    for i in range(0, num_sequences, batch_size):
        ...
        batch_embeddings = _embed_batch_grover(batch_seqs, tokenizer, model, device, max_length)
...
for folder in folders:
    for file_type in file_types:
        input_file_path = f"{project_dir}/data_processed/causal/{folder}/{file_type}_seqs_short.csv"
        ...
        embeddings1 = get_grover_embeddings(model, tokenizer, device, sequences1, BATCH_SIZE, MAX_LENGTH)

        print(f"Generating embeddings for {num_seqs} sequences in Seq2...")
        embeddings2 = get_grover_embeddings(model, tokenizer, device, sequences2, BATCH_SIZE, MAX_LENGTH)
