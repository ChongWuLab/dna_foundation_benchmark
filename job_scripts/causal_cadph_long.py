...
def mean_pooling(hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    ...
    embed = torch.sum(hidden_states * attention_mask, dim=1)
    embed = embed / torch.sum(attention_mask, dim=1)
    return embed

def _embed_batch_with_rc(seq_list, tokenizer, model, max_length):
    """
    Internal function to embed a single batch of sequences by averaging
    forward and reverse-complement passes.
    """
    ...
    embed = mean_pooling(hidden_avg, attn_mask)

    return embed

def get_cadph_embeddings(model, tokenizer, sequences, batch_size, max_length):
    """
    ...
    for i in range(0, num_sequences, batch_size):
        ...
        batch_embeddings = _embed_batch_with_rc(batch_seqs, tokenizer, model, max_length)
        ...
        print(f"Batch {(i // batch_size) + 1} / {num_batches} processed.")
...
folders = ['eqtl','paqtl', 'sqtl' ,'ipaqtl']
file_types = ['neg', 'pos']
...
for folder in folders:
    for file_type in file_types:
        ...
        # Read the input file (assumes file exists) and then check for identifier columns.
        ...
        print(f"Generating embeddings for {num_seqs} sequence 1...")
        embeddings1 = get_cadph_embeddings(model, tokenizer, cropped_seqs1, BATCH_SIZE, INPUT_SEQUENCE_LENGTH)

        print(f"Generating embeddings for {num_seqs} sequence 2...")
        embeddings2 = get_cadph_embeddings(model, tokenizer, cropped_seqs2, BATCH_SIZE, INPUT_SEQUENCE_LENGTH)
