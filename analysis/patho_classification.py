...
def load_and_prepare_data(model_name: str) -> tuple:
    ...
    X = df_combined.iloc[:, :-5].values
    y = df_combined['label'].values
    chromosomes = df_combined.iloc[:,-5]
...
X_all, y_all, chromosomes_all = load_and_prepare_data(args.model)

all_results = []

for scenario_idx, scenario in enumerate(test_scenarios):
    print(f"SCENARIO {scenario_idx + 1}: {scenario['description']}")
